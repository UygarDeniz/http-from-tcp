...
func TestBodyParse(t *testing.T) {
	...
	// Test: No Content-Length but Body Exists (shouldn't error; we assume Content-Length will be present if a body exists)
	reader = &chunkReader{
		data: "POST /submit HTTP/1.1\r\n" +
			"Host: localhost:42069\r\n" +
			"\r\n" +
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	// Body should be empty since no Content-Length was specified
	assert.Equal(t, "", string(r.Body))
}
